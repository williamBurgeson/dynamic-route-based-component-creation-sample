// from https://github.com/angular/angular/blob/master/packages/router/src/recognize.ts
...
import { getNewRouterStateSnapshot } from './ng-object-factory-functions/get-new-router-state-snapshot';
import { getNewActivatedRouteSnapshot } from './ng-object-factory-functions/get-new-activated-route-snapshot';
import { TreeNode } from './tree';
import { inheritedParamsDataResolve } from './router_state';
import { mapChildrenIntoArray } from "./url_tree";
import { defaultUrlMatcher } from "./shared";
import { forEach, last } from "./collection";
...
class Recognizer {
  ...
  recognize(): Observable<RouterStateSnapshot> {
    try {
      ...
      const root = getNewActivatedRouteSnapshot(
      ...
      const routeState = getNewRouterStateSnapshot(this.url, rootNode);

      // the code in the original is here, commented out; not sure if it will work or not
      // this.inheritParamsAndData(routeState._root);

      this.inheritParamsAndData(rootNode);
  ...
  processSegmentAgainstRoute(
    ...
    if (route.path === '**') {
      ...
      const snapshot = getNewActivatedRouteSnapshot(
